...
function download_formats()
	...
	mp.osd_message("fetching available formats with yt-dlp/youtube-dl...", 60)
	...
	if not ytdl.searched then
		...
		if not (ytdl_mcd == nil) then
			...
			ytdl_mcd = mp.find_config_file("youtube-dl")
			if not (ytdl_mcd == nil) then
	...
	if (es < 0) or (json == nil) or (json == "") then
		...
		msg.error("failed to get format list: exit code " .. es)
		return {}, 0
	end
