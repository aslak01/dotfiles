...
return {
    ...
    linters = {
      ...
      selene = {
        condition = function(ctx)
          return #vim.fs.find(
            "selene.toml",
            { path = ctx.filename, upward = true, type = "file" }
          ) > 0
        end,
      },
    },
  ...
  config = function(_, opts)
    ...
    for name, linter in pairs(opts.linters or {}) do
      lint.linters[name] = (
        type(linter) == "table" and type(lint.linters[name]) == "table"
      )
    ...
    lint._resolve_linter_by_ft = astrocore.patch_func(
      lint._resolve_linter_by_ft,
      function(orig, ...)
