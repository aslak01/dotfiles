...
local function get_kind_icon(CTX)
  ...
  if not icon_provider then
    local base = function(ctx)
      ctx.kind_icon_highlight = "BlinkCmpKind" .. ctx.kind
    end
    ...
    if _G.MiniIcons then
      icon_provider = function(ctx)
        ...
        elseif ctx.item.source_name == "Path" then
          ctx.kind_icon, ctx.kind_icon_highlight = mini_icons.get(
            ctx.kind == "Folder" and "directory" or "file",
            ctx.label
          )
        end
      end
  ...
  if not hl_provider then
    local highlight_colors_avail, highlight_colors =
  ...
  return {
    ...
    highlight = CTX.kind_icon_highlight,
  }
end
