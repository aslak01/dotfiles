...
-- yt-dl may set special http headers for some sites (user-agent, cookies)
...
local function edl_track_joined(fragments, protocol, is_live, base)
    ...
    if (protocol == "http_dash_segments") and not fragments[1].duration and not is_live then
        -- assume MP4 DASH initialization segment
        table.insert(parts, "!mp4_dash,init=" .. edl_escape(join_url(base, fragments[1])))
        offset = 2
...
local function add_single_video(json)
    ...
    elseif reqfmts then
        for _, track in pairs(reqfmts) do
            ...
            elseif track.vcodec == "none" then
                ...
                mp.commandv("audio-add", edl_track or track.url, "auto", track.format_note or "")
            end
        end
    ...
    if not (json.stretched_ratio == nil) and not option_was_set("video-aspect") then
...
function run_ytdl_hook(url)
    ...
    -- check for yt-dl in mpv's config dir
    ...
    if (mp.get_property("options/vid") == "no") and not option_was_set("ytdl-format") then
        ...
        then
            ...
            if not (entry_wsubs == nil) and not (json.entries[entry_wsubs].duration == nil) then
                for j, req in pairs(json.entries[entry_wsubs].requested_subtitles) do
                    ...
                    for i, entry in pairs(json.entries) do
                        if
                            not (entry.requested_subtitles == nil)
                            and not (entry.requested_subtitles[j] == nil)
